...
public class RenovarLicenciaController implements Initializable {
    ...
    private void renovar(ActionEvent event) {
        try {
            App.gestor.RenovarLicencia(licencia);
            ImprimirLicencia();
    ...
    private void setDatos() throws Exception{
        ...
        int plazo = App.gestor.CalcularVigenciaLicencia(licencia.titular, licencia.clase);
        fechaInicio = LocalDate.now();
        fechaFinal = LocalDate.of(fechaInicio.plusYears(plazo).getYear(), licencia.titular.fechaDeNacimiento.getMonth(),
            licencia.titular.fechaDeNacimiento.getDayOfMonth());
